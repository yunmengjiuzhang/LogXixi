...
//        autoClear(5);
    ...
    private boolean handleException(Throwable ex) {
        if (ex == null || !LogXixi.isApkInDebug())
            return false;
        try {
            // 使用Toast来显示异常信息
            new Thread() {
                ...
                public void run() {
                    Looper.prepare();
                    Toast.makeText(LogXixi.getContext(), "很抱歉,程序出现异常,即将重启.", Toast.LENGTH_LONG).show();
                    Looper.loop();
                }
            }.start();
            // 收集设备参数信息
    ...
    public void collectDeviceInfo() {
        try {
            PackageManager pm = LogXixi.getContext().getPackageManager();
            PackageInfo pi = pm.getPackageInfo(LogXixi.getContext().getPackageName(), PackageManager.GET_ACTIVITIES);
            if (pi != null) {
    ...
    private void writeFile(String sb) throws Exception {
        if (Environment.getExternalStorageState()
            ...
            File dir = new File(LogXixi.getDirPath());
            if (!dir.exists())
                dir.mkdirs();

            FileOutputStream fos = new FileOutputStream(LogXixi.getDirPath() + LogXixi.getCrashFileName(), true);
